...
namespace Trakx.Contracts.Set.SetTokenFactory
{
    ...
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, SetTokenFactoryDeployment setTokenFactoryDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
        ...
        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, SetTokenFactoryDeployment setTokenFactoryDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<SetTokenFactoryDeployment>().SendRequestAsync(setTokenFactoryDeployment);
        }

        public static async Task<SetTokenFactoryService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, SetTokenFactoryDeployment setTokenFactoryDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
        ...
        public SetTokenFactoryService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
        ...
        public Task<string> CreateSetRequestAsync(List<string> components, List<BigInteger> units, BigInteger naturalUnit, byte[] name, byte[] symbol, byte[] returnValue6)
        {
            var createSetFunction = new CreateSetFunction();
                ...
                createSetFunction.ReturnValue6 = returnValue6;
            
             return ContractHandler.SendRequestAsync(createSetFunction);
        }

        public Task<TransactionReceipt> CreateSetRequestAndWaitForReceiptAsync(List<string> components, List<BigInteger> units, BigInteger naturalUnit, byte[] name, byte[] symbol, byte[] returnValue6, CancellationTokenSource cancellationToken = null)
        {
            var createSetFunction = new CreateSetFunction();
                createSetFunction.Components = components;
                createSetFunction.Units = units;
