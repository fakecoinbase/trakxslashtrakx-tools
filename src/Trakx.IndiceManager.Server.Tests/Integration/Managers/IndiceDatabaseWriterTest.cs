...
{ 
    public sealed class IndiceDatabaseWriterTest : IDisposable
    {
        ...
        {
            var savedComposition = new IndiceCompositionDao(new IndiceDefinitionDao("TRX", null, null, 8, "published address", null), 2, DateTime.Now, null, null);
            ...
            await _indiceDataCreator.DidNotReceiveWithAnyArgs().AddNewComposition(default,default);
        }

        [Fact]
        public async Task TrySaveComposition_should_not_modify_published_composition()
        {
            var publishedComposition = new IndiceCompositionDao(new IndiceDefinitionDao("TRX", "Trakx test", null, 8, "published address", null), 2, DateTime.Now, "published address", null);
